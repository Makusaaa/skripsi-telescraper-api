...
export async function updateAlarmNotes(db: DB, alarmid: number, notes: string){
    return (await db.update(alarms).set({ notes: notes }).where(eq(alarms.alarmid,alarmid)).returning())[0];
}
